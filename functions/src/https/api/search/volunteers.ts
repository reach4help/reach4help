    ...
    if (userId) {
      ...
      if (user && user.applicationPreference) {
        ...
        for (const queryResult of queryResults.docs) {
          idToDataMapping[queryResult.id] = PrivilegedUserInformation.factory(queryResult.data() as IPrivilegedUserInformation).toObject() as IPrivilegedUserInformation;
        }
        return {
          sucess: true,
