...
import * as data from './reach4helpDataset.json';

const orgs = JSON.parse(JSON.stringify(data as any));
...
export class MarkerData {
      ...
      contact: {
        ...
        volunteers: {
          web: {
            ...
            'Donate Funds':
              'https://servingseniors.org/get-involved/donate.html',
          },
    ...
    },
    {
      ...
      contact: {
        ...
        volunteers: {
          web: {
            'Area Form':
              'https://barremutualaid.recovers.org/volunteer_info/new',
            'Town Form':
  ...
  private parseJSONMarkerData() {
    for (const org of orgs) {
      if (org !== undefined) {
        // Using assertive type instead of type casting since
        // JSON data is definitively going to follow the MarkerInfo
        // interface model
        this.MARKERS.push(org as MarkerInfo);
      }
    }
  }

  public getMarkerData(): MarkerInfo[] {
    this.parseJSONMarkerData();
